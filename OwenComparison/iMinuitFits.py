...
n_bootstraps = 500
epsilon = 1e-8
rcond = 1e-3  # Define rcond for pseudo-inverse
...
def compute_fits(smearing):
    try:
        ...
        fitted_errors_cov = np.empty((n_bootstraps, n_params))
        fitted_params_cov_diag = np.empty((n_bootstraps, n_params))
        fitted_errors_cov_diag = np.empty((n_bootstraps, n_params))
        
        bin_mids = 0.5 * (bins[:-1] + bins[1:])

        results = [['Parameter', 'Value', 'Error', 'Fit_Type', 'Bootstrap_Index']]
        ...
        for i in range(n_bootstraps):
            ...
            try:
                m.minos()
                merrors = m.merrors
                mu_err = merrors["mu"].upper
                var_err = merrors["var"].upper
            except RuntimeError:
                mu_err = np.nan
                var_err = np.nan
                
            # Store the fitted parameters and errors
            fitted_params_cov[i, :] = [m.values["mu"], m.values["var"]]
            fitted_errors_cov[i, :] = [mu_err, var_err]
            ...
            try:
                ...
                mu_err_diag = merrors_diag["mu"].upper
                var_err_diag = merrors_diag["var"].upper
            except RuntimeError:
                mu_err_diag = np.NaN
                var_err_diag = np.NaN
            
            # Store the fitted parameters and errors for cov_diag
            fitted_params_cov_diag[i, :] = [m_diag.values["mu"], m_diag.values["var"]]
            fitted_errors_cov_diag[i, :] = [mu_err_diag, var_err_diag]

            # Results from the fit using cov
            results.append(['mu', fitted_params_cov[i, 0], fitted_errors_cov[i, 0], 'cov', i])
            results.append(['var', fitted_params_cov[i, 1], fitted_errors_cov[i, 1], 'cov', i])
            
            # Results from the fit using cov_diag
            results.append(['mu', fitted_params_cov_diag[i, 0], fitted_errors_cov_diag[i, 0], 'cov_diag', i])
            results.append(['var', fitted_params_cov_diag[i, 1], fitted_errors_cov_diag[i, 1], 'cov_diag', i])
        ...
        mu_rms_cov  = np.std(fitted_params_cov[:, 0], ddof=1)
        var_rms_cov = np.std(fitted_params_cov[:, 1], ddof=1)
        
        with open(filename, 'a', newline='') as csvfile:
            csvwriter = csv.writer(csvfile)
            csvwriter.writerow([])
            csvwriter.writerow(["# RMS of Best-Fit Parameters Across Bootstraps"])
            csvwriter.writerow(["Parameter", "RMS_Cov", "RMS_CovDiag"])
            csvwriter.writerow(["mu",  f"{mu_rms_cov:.6g}"])
            csvwriter.writerow(["var", f"{var_rms_cov:.6g}"])
            print(f"RMS of best-fit parameters appended to {filename}")
